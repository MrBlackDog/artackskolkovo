...
public class ImprovedOrientationSensor2Provider extends OrientationProvider {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_ROTATION_VECTOR) {
            ...
            // Store in quaternion
            quaternionRotationVector.set(temporaryQuaternion[1], temporaryQuaternion[2], temporaryQuaternion[3], temporaryQuaternion[0]);
            synchronized (synchronizationToken2) {
            ...
            if (timestamp != 0) {
                ...
                deltaQuaternion.set((float) (sinThetaOverTwo * axisX),(float) (sinThetaOverTwo * axisY),(float) (sinThetaOverTwo * axisZ),(float) cosThetaOverTwo);

                // Move current gyro orientation
                quaternionGyroscope.mul(deltaQuaternion);
                ...
                float dotProd = quaternionGyroscope.dot(quaternionRotationVector);
                ...
                if (Math.abs(dotProd) < OUTLIER_THRESHOLD) {
                    ...
                    quaternionGyroscope.slerp(quaternionRotationVector,
    ...
    private void setOrientationQuaternionAndMatrix(Quaternion quaternion) {
        synchronized (synchronizationToken) {
            // Use gyro only
            currentOrientationQuaternion.set(quaternion);
        }
    }
}
